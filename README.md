...
처음 시작하고 계획을 세울 때만 해도 요구사항에 있는 부분들을 마무리하고 아이디어 낸 기능들을 구현할 수 있을까 걱정이 많았습니다. 프로젝트 동안에는 최대한 처음 세웠던 일정대로 그날 계획한 부분은 그날 마무리 지으려고 노력했고 이 과정에서 팀장님과 새벽 4시까지 작업하기도 하고 팀장님은 밤을 새며 작업하기도 했습니다. 차근차근 하나씩 구현해나가다보니 프로젝트 마지막날 드디어 계획했던 모습 그대로 배포까지 마무리할 수 있었습니다. 
프로젝트가 끝나가는 지금 프로젝트를 진행하는데 있어서 가장 중요한 것은 처음 설계와 소통이라 생각합니다. 저희는 프로젝트를 시작하기 전 어떻게 나아갈 지 방향성과 구현해내고 싶은 아이디어들을 위해 충분히 이야기를 나눴고 진행하면서도 계속 확인했기에 함께 큰 문제없이 프로젝트를 진행할 수 있었다 생각하기 때문입니다.
프로젝트를 하면서 어려움도 많았습니다. 오류가 나서 오류때문에 하루를 허비한 적도 있었고 각자 branch로 작업하다 merge하는 과정에서 어려움을 겪기도 했지만 그때마다 다시 공부했던 것을 돌아보고 google 검색을 통해 생각을 더 넓히기도 하고 교수님과 주위 분들에게 물어가며 해결할 수 있었습니다. 프로젝트를 통해 여러 기능들을 구현했는데 하나씩 했을 때는 이상없던 부분들이  합쳤을 때는 꼬여서 오류가 나는 것을 보고 좀 더 크게 볼 줄 알아야겠다 생각했습니다. 또한, 화면으로 봤을 때는 간단해 보이는 기능의 구현도 실제로 구현해내는 과정은 생각보다 간단하지 않은 경우가 많았기에 앞으로도 많이 공부해서 다음부터는 더 잘해내고 싶다 생각했습니다. 
프로젝트를 완성해가며 뿌듯함도 많이 느꼈고 해냈다는 성취감도 많이 느꼈습니다. 혼자했으면 절대 이 기간안에 아이디어를 다 구현할 수 없었을텐데 프로젝트 내내 저를 이끌고 프로젝트를 진행해주신 수한님께 정말 감사했습니다. 아이디어를 구현해내고 branch로 작업하는 모습을 보고 정말 개발자로 가는 길에 서있구나 느낄 수 있었던 프로젝트였던 것 같습니다. 이번 프로젝트를 계기로 앞으로 더 발전해나아가고 싶습니다. 
